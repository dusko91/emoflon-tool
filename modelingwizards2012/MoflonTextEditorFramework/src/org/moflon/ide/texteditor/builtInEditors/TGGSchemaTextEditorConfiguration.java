 ...
 * functionality.
 */
public class TGGSchemaTextEditorConfiguration extends MoflonTextEditorConfiguration
{      
   private HashMap<String, MoflonEditorTemplate> contextFreeTemplates = new HashMap<String,MoflonEditorTemplate>();
   
   private Logger logger = Logger.getLogger(TGGSchemaTextEditorConfiguration.class);
   
   public TGGSchemaTextEditorConfiguration(){
      ...
      try
      {
         ...
         URL resource = bundle.getResource("resources/moca/templates/texteditor/tggschemaTemplates/tggschema_templates.properties");
   ...
   {      
      // Recognize integration name
      if( getWord(2).equals("<") && getWord(1).equals("{") ){
         highlight(COLORS.VIOLET, BOLD);
      }
      
      if(getWord(-1).equals("."))
         highlight(COLORS.BLUE,ITALIC);
      else if(getWord(-1).equals("extends"))
         highlight(COLORS.DARKBLUE,BOLD);
      
      if(getWord(1).equals("{") && (getWord(2).equals("source")||getWord(2).equals("target")))
         highlight(COLORS.DARKBLUE,BOLD);
      else if(getWord(1).equals("extends"))
         highlight(COLORS.DARKBLUE,BOLD);

      // Error highlighting if some key elements are used in an unappropriated place
   }
   ...
   public Collection<MoflonEditorTemplate> getTemplates()
   {
      ...
      // Context sensitive Templates first, then context insensitive
      if(getWord(-2).equals("\"") && getWord(-3).equals("->") && getWord(-4).equals("uri"))
      {
         MoflonEditorTemplate moflonUriTemplate = new MoflonEditorTemplate("Moflon uri", "Creates a new default moflon uri",
               MoflonUtil.getMoflonDefaultURIForProject(getWord(-1)), 0);
         templates.add(moflonUriTemplate);
      }
