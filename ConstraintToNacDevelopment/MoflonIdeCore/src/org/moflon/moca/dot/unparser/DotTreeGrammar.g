...
rootGraph:^('GRAPH' name=. (entries += graph | entries += node)*) -> root_graph(name={$name}, entries={$entries});

graph: ^('GRAPH' name=. (entries += graph | entries += node)*) -> graph_lines(name={$name}, entries={$entries});

node: ^('NODE' name=. argument+=args?) -> node_line(name={$name}, argument={$argument});

args: ^('ARGS' arguments+=.+) -> args_line(arguments={$arguments});
