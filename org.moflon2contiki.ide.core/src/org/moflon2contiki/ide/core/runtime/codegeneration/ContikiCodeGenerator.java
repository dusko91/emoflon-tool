package org.moflon2contiki.ide.core.runtime.codegeneration;
...
import org.moflon2contiki.ide.core.runtime.codegeneration.HeaderFileGenerator.BuiltInTypes;
import org.moflon2contiki.ide.core.runtime.codegeneration.utilities.ContikiStringRenderer;
import org.moflon2contiki.ide.core.runtime.codegeneration.utilities.FieldAttribute;
import org.moflon2contiki.ide.core.runtime.codegeneration.utilities.MethodAttribute;
import org.moflon2contiki.ide.core.runtime.codegeneration.utilities.Type;
import org.moflon2contiki.ide.core.runtime.codegeneration.utilities.ContikiIncludes.Components;
...
public class ContikiCodeGenerator {
	...
	public ContikiCodeGenerator(Resource ecore, IProject project, GenModel genModel,
		...
		this.constantProperties = Moflon2ContikiWorkspaceHelper.getConstantsPropertiesFile(project);
		...
		this.mapping = Moflon2ContikiWorkspaceHelper.getMappingPropertiesFile(project);
	...
	public IStatus generateCode(IProgressMonitor eclipseSubProgress) {
				...
				for (GenOperation genOperation : genClass.getGenOperations()) {
					...
					if (!generatedMethodBody.equals(MoflonUtil.DEFAULT_METHOD_BODY)) {
						...
						methods.add(new MethodAttribute(
								...
								new Type(isBuiltInType(genOperation.getEcoreOperation().getEType().getName()),
										genOperation.getEcoreOperation().getEType().getName()),
		...
		generateHeader(component, algorithmName, null, fields, methods,
				Moflon2ContikiWorkspaceHelper.createSubmonitorWith1Tick(eclipseSubProgress));

		// generate "static" code for source file
		generateSourceFile(component, algorithmName, generatedCode, genClassesForInjectedCode,
				Moflon2ContikiWorkspaceHelper.createSubmonitorWith1Tick(eclipseSubProgress));
	...
	private Collection<? extends FieldAttribute> getFields(GenClass genClass) {
		...
		for (EReference ref : clazz.getEAllReferences()) {
			fields.add(new FieldAttribute(new Type(isBuiltInType(clazz.getName()), clazz.getName()),
					...
					ref.getName(), ref.getUpperBound() < 0));
		}
	 ...
	 */
	protected void generateSourceFile(String component, String algorithmName, String generatedCode,
		...
		source.registerRenderer(String.class, new ContikiStringRenderer());
		...
		try {
			Moflon2ContikiWorkspaceHelper.addFile(project,
					Moflon2ContikiWorkspaceHelper.GEN_FOLDER + "/" + filename + ".c", contents,
					Moflon2ContikiWorkspaceHelper.createSubmonitorWith1Tick(iProgressMonitor));
	...
	protected void generateHeader(String componentName, String algorithmName, EList<EOperation> operations,
		...
		// Generate Header Def
		stg.registerRenderer(String.class, new ContikiStringRenderer());
		...
		try {
			Moflon2ContikiWorkspaceHelper.addFile(project,
					Moflon2ContikiWorkspaceHelper.GEN_FOLDER + "/" + componentName + "-" + algorithmName + ".h",
					contents, Moflon2ContikiWorkspaceHelper.createSubmonitorWith1Tick(monitor));
		} catch (CoreException e) {
	...
	private String getParametersFromEcore(EOperation op) {
		...
		template.add("type",
				new Type(isBuiltInType(op.getEContainingClass().getName()), op.getEContainingClass().getName()));
		result += template.render();
