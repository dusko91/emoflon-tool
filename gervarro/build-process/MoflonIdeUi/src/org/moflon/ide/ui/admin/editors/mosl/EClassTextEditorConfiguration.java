...
public class EClassTextEditorConfiguration extends AbstractMOSLTextEditorConfiguration
{
   ...
   public Collection<MoflonEditorTemplate> getTemplates()
   {
      ...
      // Add basic templates
      templates.add(new MoflonEditorTemplate("reference", "EReference", "${diamond}-> ${name}(${lower}..${upper}) : ${object}", sortCounter++));
      templates.add(new MoflonEditorTemplate("attribute", "EAttribute", "${variable} : ${type}", sortCounter++));
      templates.add(new MoflonEditorTemplate("operation", "EOperation", "${name}(${args}) : ${type} {\n}", sortCounter++));

      templates.add(new MoflonEditorTemplate("if", "Simple if statement", "if [${pattern}] {\n \t[${then}]\n \t } else {\n\t [${else}]\n\t}", sortCounter++));
