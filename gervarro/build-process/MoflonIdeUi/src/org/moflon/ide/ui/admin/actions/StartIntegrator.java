...
public class StartIntegrator implements IWorkbenchWindowActionDelegate
{
   ...
   public void run(IAction action)
   {
      ...
      try
      {
         ...
         ILaunchConfigurationWorkingCopy workingCopy = type.newInstance(null, "Start Integrator");
         ...
         workingCopy.setAttribute(IJavaLaunchConfigurationConstants.ATTR_MAIN_TYPE_NAME, "org.moflon.tie.StartIntegrator");

         // Create arguments for call to main
         workingCopy.setAttribute(IJavaLaunchConfigurationConstants.ATTR_PROGRAM_ARGUMENTS, "\"" + correspondenceModel.getLocation() + "\"");
   ...
   public void selectionChanged(IAction action, ISelection selection)
   {
      try
      {
         ...
         if (selection != null && selection instanceof StructuredSelection)
         {
            ...
            {
               correspondenceModel = (IFile) fileSelection.getFirstElement();
               if (correspondenceModel != null && correspondenceModel.getFileExtension().equals("xmi"))
               {
                  // Assign project
   ...
   private void generateStartIntegratorClass()
   {
      ...
      {
         generator.doFinish();
         URL url = Activator.getPathRelToPlugIn("resources/moca/templates/TiE/EmptyBreakPointSet.xmi", UIActivator.PLUGIN_ID);
         if(!project.getFile("instances/BreakPointSet.xmi").exists())
            WorkspaceHelper.addFile(project, "instances/BreakPointSet.xmi", url, UIActivator.PLUGIN_ID, new NullProgressMonitor());
      } catch (Exception e)
      {
         e.printStackTrace();
      }
